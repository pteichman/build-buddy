# $Id: Simple.pm 3068 2005-12-22 03:41:13Z v_thunder $

# Modified by Dan Mills <thunder@ximian.com> - just the package name
# (to include it in the ximian perl distribution).

package Ximian::XML::Simple;

=head1 NAME

Ximian::XML::Simple - Easy API to maintain XML (esp config files)
...
Or the object oriented way:
    ...
    my $xs = new Ximian::XML::Simple(options);
...
To catch common errors:
    ...
    use Ximian::XML::Simple qw(:strict);
...
sub XMLin {
  ...
  my $self;
  if($_[0]  and  UNIVERSAL::isa($_[0], 'Ximian::XML::Simple')) {
    $self = shift;
  }
  else {
    $self = new Ximian::XML::Simple();
  }
...
#
# Ximian::XML::Simple requires the services of another module that knows how to
...
The Ximian::XML::Simple module provides a simple API layer on top of an underlying XML
...
allows them to be called as methods of an B<Ximian::XML::Simple> object.  The object
...
used when working with namespaces (B<Ximian::XML::Simple> can only usefully work with
namespaces when teamed with a SAX Parser).

You can use other punctuation characters in hash values (just not in hash
keys) however B<Ximian::XML::Simple> does not support dumping binary data.
...
back in.  (A later version of B<Ximian::XML::Simple> might take a more proactive
approach).
...
B<Ximian::XML::Simple> supports a number of options (in fact as each release of
B<Ximian::XML::Simple> adds more options, the module's claim to the name 'Simple'
...
When parsing from a named file, B<Ximian::XML::Simple> supports a number of caching
...
in memory in the B<Ximian::XML::Simple> module's namespace.  Subsequent calls to parse
...
When you use an B<Ximian::XML::Simple> object as a SAX handler, it will return a
...
B<Ximian::XML::Simple> object and a reference to the data tree.  The return value from
...
Newer versions of Ximian::XML::Simple sort elements and attributes alphabetically (*),
...
to override methods in B<Ximian::XML::Simple> to provide customised behaviour

=back

The default values for the options described above are unlikely to suit
everyone.  The OO interface allows you to effectively override B<Ximian::XML::Simple>'s
defaults with your preferred values.  It works like this:

First create an Ximian::XML::Simple parser object with your preferred defaults:

  my $xs = new Ximian::XML::Simple(ForceArray => 1, KeepRoot => 1);
...
If you import the B<Ximian::XML::Simple> routines like this:
...
From version 1.08_01, B<Ximian::XML::Simple> includes support for SAX (the Simple API
...
B<Ximian::XML::Simple> can operate at either end of a SAX pipeline.  For example,
...
SAX pipeline using the 'Handler' option on C<XMLout()>:
  ...
  my $simple = Ximian::XML::Simple->new(Handler => $filter);
  $simple->XMLout($ref);

You can also put B<Ximian::XML::Simple> at the opposite end of the pipeline to take
...
isolated through filtering:
  ...
  my $simple = Ximian::XML::Simple->new(ForceArray => 1, KeyAttr => ['partnum']);
...
You can build a filter by using an Ximian::XML::Simple object as a handler and setting
...
modifies it and sends it off as SAX events to a downstream handler:
  ...
  my $writer = XML::SAX::Writer->new();
  my $filter = Ximian::XML::Simple->new(
                 DataHandler => sub {
                                  my $simple = shift;
                                  my $data = shift;
...
If you don't care which parser module B<Ximian::XML::Simple> uses then skip this
section entirely (it looks more complicated than it really is).

B<Ximian::XML::Simple> will default to using a B<SAX> parser if one is available or
...
$Ximian::XML::Simple::PREFERRED_PARSER to contain the module name.  The following rules
are used:
...
The package variable takes precedence over the environment variable if both are defined.  To force B<Ximian::XML::Simple> to ignore the environment settings and use
...
the whole document to be rejected.  B<Ximian::XML::Simple> will die with an appropriate
...
Ximian::XML::Simple is not the right tool for your job - check out the next section.

=head1 WHERE TO FROM HERE?

B<Ximian::XML::Simple> is able to present a simple API because it makes some
...
B<Ximian::XML::Simple> requires either L<XML::Parser> or L<XML::SAX>.
...
Answers to Frequently Asked Questions about Ximian::XML::Simple are bundled with this
distribution as: L<Ximian::XML::Simple::FAQ>
